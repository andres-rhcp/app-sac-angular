...
@Component({
  selector: 'app-ficha-recurso',
  templateUrl: './ficha-recurso.component.html',
  styleUrls: ['./ficha-recurso.component.css']
})
export class FichaRecursoComponent implements OnInit {
  ...
  rec_campo_2: string;
  constructor(
    public dialogRef: MatDialogRef<FichaRecursoComponent>,
  ...
  ngOnInit() {
    var data = this.servidorService.getRecursoById(this.data.id).pipe(first()).subscribe(data => {
      ...
      data.forEach(recurso => {
        ...
        this.rec_autor_personal = recurso.rec_autor_personal.toUpperCase();
