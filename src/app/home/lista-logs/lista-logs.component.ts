...
@Component({
  selector: 'app-lista-logs.component',
...
export class ListaLogsComponent implements OnInit {
  ...
  cargarListaTablas(evt?: any) {
    ...
    this.servidorService.getAllPagosByEstado('REVISADO')
      .pipe(first())
      .subscribe(
        respuesta => {
  ...
  openAceptarDialog(element): void {
    const dialogRef = this.dialog.open(DialogComponent, {
      width: '340px',
      data: { title: 'Aceptar', message: '¿Está seguro de realizar este registro de pago?' },
      panelClass: 'my-class'
    });
    dialogRef.afterClosed().subscribe(result => {
      if (result == 'OK') {
